...
- **No `emitDecoratorMetadata`, anywhere.** tsup/esbuild and Vitest's esbuild transform cannot emit it, so nothing in this repo may rely on `design:paramtypes`: every injectable uses explicit `@Inject(TOKEN)` on constructor parameters (including test controllers), and only `experimentalDecorators: true` is enabled (`packages/idempotix-nestjs/tsconfig.json`). This is why there is no `unplugin-swc`/`@swc/core` in the toolchain.
- `@sabeesoft/idempotix-nestjs` deliberately has no `nestjs-cls` dependency: transactions come in through the `runInTransaction` option (`(fn) => host.withTransaction(fn)`), and the store gets a `client: () => host.tx` callback. NestJS metadata keys (`'path'`, `'method'`, `'__httpCode__'`) are used as literal strings because their export path moved between Nest majors while the values did not.
